...
class Inferer:
    ...
    def __call__(
        ...
        """
        Generate an answer from the llama model using the given question and choices.

        Args:
            choices (str):
                The 4 choices to provide to the model.
        ...
        prompt = self.data_handler.generate_prompt(question=question, choices=choices, answer=answer)
        if verbose:
            print(prompt)
        ...
        split = f'{self.data_handler.prompt_template["answer"]}{answer or ""}'
